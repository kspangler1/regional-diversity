...
## Transformation 
div_12$simpson_new_transf <- (div_12$simpson_new)^2
div_17$simpson_new_transf <- (div_17$simpson_new)^2

skewness(div_12$simpson_new_transf, na.rm = TRUE) #after to transformation
skewness(div_17$simpson_new_transf, na.rm = TRUE) #after to transformation

hist(div_12$simpson_new_transf)
hist(div_17$simpson_new_transf)

div_12_sidi_transf <- div_12 %>% dplyr::select(-c("GEOID", "YEAR", "shannon_new", "richness_new", "simpson_new"))
div_17_sidi_transf <- div_17 %>% dplyr::select(-c("GEOID", "YEAR", "shannon_new", "richness_new", "simpson_new"))
```

shannon_new is normally distributed and does not need to be transformed.  
simpson_new is skewed to the left.
richness_new is normally distributed and does not need to be transformed.
We transformed simpson_new by the square transformation to simpson_new_transf
...
rf12_sidi_transf <- randomForest(simpson_new_transf ~ ., data = div_12_sidi_transf, ntree = 400, mtry = 13) #specify # of variables and trees for splitting here
rf12_sidi_transf
```
...
### What are the most important variables?
```{r}
imp <- as.data.frame(varImpPlot(rf12_sidi_transf))
```
...
saveRDS(imp, file = paste0('./results/RF12_SIDITRANSFimp_nation.RDS'))
ggsave(imp_plot, file = paste0('./figs/RF12_SIDITRANSFimpplot_nation.png'))
```
...
rf17_sidi_transf <- randomForest(simpson_new_transf ~ ., data = div_17_sidi_transf, ntree = 400, mtry = 13) #specify # of variables and trees for splitting here
rf17_sidi_transf
```

### What are the most important variables?
```{r}
imp <- as.data.frame(varImpPlot(rf17_sidi_transf))
```
...
saveRDS(imp, file = paste0('./results/RF17_SIDITRANSFimp_nation.RDS'))
ggsave(imp_plot, file = paste0('./figs/RF17_SIDITRANSFimpplot_nation.png'))
```
...
rf17_siditransf_tune1 <- randomForest(simpson_new_transf ~ ., data = div_17_sidi_transf, ntree = 1000) #default mtry = p/3
rf17_siditransf_tune1

set.seed(1234)
rf17_siditransf_tune2 <- randomForest(simpson_new_transf ~ ., data = div_17_sidi_transf, ntree = 1000, mtry = 24) #mtry = (p/3)*2
rf17_siditransf_tune2

set.seed(1234)
rf17_siditransf_tune3 <- randomForest(simpson_new_transf ~ ., data = div_17_sidi_transf, ntree = 2000) #default mtry = p/3
rf17_siditransf_tune3

set.seed(1234)
rf17_siditransf_tune4 <- randomForest(simpson_new_transf ~ ., data = div_17_sidi_transf, ntree = 2000, mtry = 24) #mtry = (p/3)*2
rf17_siditransf_tune4

```

#### Variable importance 
```{r}
imp1 <- as.data.frame(varImpPlot(rf17_siditransf_tune1))
imp2 <- as.data.frame(varImpPlot(rf17_siditransf_tune2))
imp3 <- as.data.frame(varImpPlot(rf17_siditransf_tune3))
imp4 <- as.data.frame(varImpPlot(rf17_siditransf_tune4))
```
...
ggsave(imp1_plot, file = paste0('./figs/RF17_SIDITRANSFimpplot_tune1.png'))
...
ggsave(imp2_plot, file = paste0('./figs/RF17_SIDITRANSFimpplot_tune2.png'))
...
ggsave(imp3_plot, file = paste0('./figs/RF17_SIDITRANSFimpplot_tune3.png'))
...
ggsave(imp4_plot, file = paste0('./figs/RF17_SIDITRANSFimpplot_tune4.png'))
```
...
```{r}
# remove variables that contributed least to the full model
rf12_siditransf_frr <- function(frr) {
  
  #index
  index <- frr #name data frames produced 
  
  # build data
  div_12_siditransfrf <- div_12_sidi_transf %>% 
  filter(FRR == frr) %>% #specify FRR
  dplyr::select(-c(FRR)) #get rid of FRR column
  
  # build RF on training data
  set.seed(1234) 
  frr_siditransf_12 <- randomForest(simpson_new_transf ~ ., data = div_12_siditransfrf,ntree = 2000, importance = TRUE) #specify trees and # of variables 
  frr_siditransf_12
  
  # importance plots
  imp <- as.data.frame(varImpPlot(frr_siditransf_12))
  imp$varnames <- rownames(imp) # row names to column
  ...
  # build argument to feed save, below
  list <- list("var_exp" = frr_siditransf_12)
  
  # return
  saveRDS(imp, file = paste0('./results/RF12_SIDITRANSFimp_',index,'.RDS'))
  ggsave(imp_plot, file = paste0('./figs/RF12_SIDITRANSFimpplot_',index,'.png')) # importance plots individually
  return(list) # will produce variance explained and metrics of resubstitution 

}

rf12_siditransf_1 <- rf12_siditransf_frr(1) 
rf12_siditransf_2 <- rf12_siditransf_frr(2)
rf12_siditransf_3 <- rf12_siditransf_frr(3)
rf12_siditransf_4 <- rf12_siditransf_frr(4)
rf12_siditransf_5 <- rf12_siditransf_frr(5)
rf12_siditransf_6 <- rf12_siditransf_frr(6)
rf12_siditransf_7 <- rf12_siditransf_frr(7)
rf12_siditransf_8 <- rf12_siditransf_frr(8)
rf12_siditransf_9 <- rf12_siditransf_frr(9)
...
```{r}
# remove variables that contributed least to the full model
rf17_siditransf_frr <- function(frr) {
  
  #index
  index <- frr #name data frames produced 
  
  # build data
  div_17_siditransfrf <- div_17_sidi_transf %>% 
  filter(FRR == frr) %>% #specify FRR
  dplyr::select(-c(FRR)) #get rid of FRR column
  
  # build RF on full data
  set.seed(1234) 
  frr_siditransf_17 <- randomForest(simpson_new_transf ~ ., data = div_17_siditransfrf,ntree = 2000, importance = TRUE) #specify trees and # of variables 
  frr_siditransf_17
  
  # importance plots
  imp <- as.data.frame(varImpPlot(frr_siditransf_17))
  imp$varnames <- rownames(imp) # row names to column
  ...
  # build argument to feed save, below
  list <- list("var_exp" = frr_siditransf_17)
  
  # return
  saveRDS(imp, file = paste0('./results/RF17_SIDITRANSFimp_',index,'.RDS'))
  ggsave(imp_plot, file = paste0('./figs/RF17_SIDITRANSFimpplot_',index,'.png')) # importance plots individually
  return(list) # will produce variance explained and metrics of resubstitution 

}

rf17_siditransf_1 <- rf17_siditransf_frr(1) 
rf17_siditransf_2 <- rf17_siditransf_frr(2)
rf17_siditransf_3 <- rf17_siditransf_frr(3)
rf17_siditransf_4 <- rf17_siditransf_frr(4)
rf17_siditransf_5 <- rf17_siditransf_frr(5)
rf17_siditransf_6 <- rf17_siditransf_frr(6)
rf17_siditransf_7 <- rf17_siditransf_frr(7)
rf17_siditransf_8 <- rf17_siditransf_frr(8)
rf17_siditransf_9 <- rf17_siditransf_frr(9)
